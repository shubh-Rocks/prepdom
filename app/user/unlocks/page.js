import Link from "next/link";
import { redirect } from "next/navigation";
import { Coins, Lock, ShieldCheck } from "lucide-react";
import { getAuthSession } from "@/lib/auth/session";

export const dynamic = "force-dynamic";

export default async function UnlocksPage() {
  const session = await getAuthSession();

  if (!session?.user) {
    redirect("/user/login?callbackUrl=/user/unlocks");
  }

  return (
    <div className="min-h-screen bg-[linear-gradient(160deg,#f8fafc_0%,#ecfeff_45%,#f0fdf4_100%)] px-5 py-10 sm:px-8">
      <main className="mx-auto w-full max-w-4xl rounded-3xl border border-zinc-200/80 bg-white p-6 shadow-[0_22px_80px_-28px_rgba(15,23,42,0.28)] sm:p-10">
        <p className="text-xs font-semibold uppercase tracking-[0.16em] text-zinc-500">My Unlocks</p>
        <h1 className="mt-2 text-3xl font-bold tracking-tight text-zinc-900">Unlocked papers</h1>
        <p className="mt-2 text-sm text-zinc-600">See what you unlocked with coins and revisit premium content anytime.</p>

        <section className="mt-6 grid gap-4 sm:grid-cols-3">
          <article className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
            <Lock className="h-5 w-5 text-zinc-700" />
            <p className="mt-3 text-sm font-semibold text-zinc-900">Unlock history</p>
            <p className="text-xs text-zinc-500">Every premium paper you unlocked.</p>
          </article>

          <article className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
            <Coins className="h-5 w-5 text-zinc-700" />
            <p className="mt-3 text-sm font-semibold text-zinc-900">Coins spent</p>
            <p className="text-xs text-zinc-500">Track your spending across unlocks.</p>
          </article>

          <article className="rounded-2xl border border-zinc-200 bg-zinc-50 p-4">
            <ShieldCheck className="h-5 w-5 text-zinc-700" />
            <p className="mt-3 text-sm font-semibold text-zinc-900">Access status</p>
            <p className="text-xs text-zinc-500">Verify active access to resources.</p>
          </article>
        </section>

        <section className="mt-6 rounded-2xl border border-zinc-200 p-5">
          <h2 className="text-lg font-semibold text-zinc-900">Coming next</h2>
          <p className="mt-2 text-sm text-zinc-600">
            Unlock records page is now in place. We can connect it to Unlock and CoinTransaction data in the next iteration.
          </p>
        </section>

        <div className="mt-6 flex flex-wrap gap-4 text-sm font-semibold text-zinc-700">
          <Link href="/user/dashboard" className="underline decoration-zinc-300 underline-offset-4">
            Go to Dashboard
          </Link>
          <Link href="/" className="underline decoration-zinc-300 underline-offset-4">
            Back to Home
          </Link>
        </div>
      </main>
    </div>
  );
}
